...
const LineItemsSection: React.FC<LineItemsSectionProps> = ({
  ...
  return <div className="space-y-6">
      {pdfUrl && <Collapsible open={isPdfVisible} onOpenChange={setIsPdfVisible} className="w-full">
          <div className="flex justify-between items-center mb-2">
            
            <CollapsibleTrigger asChild>
              <Button variant="outline" size="sm" className="gap-1">
                {isPdfVisible ? <><Minimize2 className="h-4 w-4" /> {t.hidePdf}</> : <><Maximize2 className="h-4 w-4" /> {t.showPdf}</>}
              </Button>
            </CollapsibleTrigger>
          </div>
